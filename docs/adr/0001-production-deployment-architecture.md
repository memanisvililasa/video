...
Phase A разворачивается на одном controlled host и не является публичным multi-user SaaS. Она включает отдельные web и worker процессы, PostgreSQL, shared durable volume и независимый от web cleanup/scheduler mechanism.
...
Scheduled cleanup command ---------- PostgreSQL + durable volume
```
...
- Выполняет job-scoped cleanup и оставляет reconciliation независимому scheduler при transient failure.

#### Cleanup/scheduler

- Может быть отдельным процессом либо контролируемой scheduled command.
...
- Не зависит от lifecycle, traffic или restart web-процесса.
...
Phase A сохраняет текущую trust-none policy:

- `Forwarded`, `X-Forwarded-For`, `X-Real-IP`, provider headers и аналогичные user-controlled forwarding headers не являются client identity.
- Неопознанные клиенты продолжают использовать общий fallback bucket.
- Общий bucket допустим только для private/controlled usage с ограниченным числом клиентов.
- Direct public access к application origin запрещён; ingress должен быть единственной разрешённой network boundary.
- Phase A нельзя объявлять публичным multi-user deployment.

Phase B требует verified ingress identity, deployment-specific ingress/provider adapter, distributed limiter store, одинакового rate-limit behavior между instances и network enforcement, закрывающего origin от обхода trusted ingress.
...
9. Cleanup, reconciliation и observability.
...
- Phase A origin закрыт ingress-ом и deployment явно обозначен private/controlled;
