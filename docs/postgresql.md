...
Migration runner читает только зафиксированный catalog `db/migrations`, берёт session advisory lock, проверяет checksum истории и применяет каждую новую migration транзакционно. Web/API requests migrations не запускают, `install`, `postinstall` и `build` hooks отсутствуют.
...
Рекомендуются отдельные роли:

- migration role: `CONNECT`, `USAGE`/`CREATE` в целевой schema и DDL только на migration window;
- runtime role: `CONNECT`, `USAGE` schema и только `SELECT`, `INSERT`, `UPDATE`, `DELETE` для `media_jobs` и `media_artifacts`; без `CREATE`, role administration и доступа к другим schemas;
- test role: права создать/удалить isolated schemas только в disposable test database.
