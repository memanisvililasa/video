...
import type {
  CanonicalInstagramContentIdentity,
...
const CAPTION_MAX_CODE_POINTS = 512;
...
/** Stage 8.6A deliberately exposes no executable or live metadata provider. */
export const INSTAGRAM_METADATA_EXECUTION_DECISION = "no-go" as const;
export const INSTAGRAM_METADATA_PROVIDER_PRODUCTION_ENABLED = false as const;

export type InstagramSafeMetadata = Readonly<{
  platform: "instagram";
  shortcode: string;
  contentType: InstagramContentKind;
  title: string;
  caption: string;
...
function assertContentType(identity: CanonicalInstagramContentIdentity, value: unknown): void {
  if (value === "image") throw metadataError(API_ERROR_CODES.IMAGE_POST_NOT_SUPPORTED);
  if (value === "carousel" || value === "mixed_carousel") {
    throw metadataError(API_ERROR_CODES.CAROUSEL_NOT_SUPPORTED);
  }
  ...
  const expected = identity.sourceKind === "reel" ? "reel" : "video_post";
  if (value !== expected) throw metadataError();
}

/**
 * Normalizes repository-controlled synthetic fixtures only. This function is
 * intentionally not an adapter: it performs no I/O and cannot invoke Instagram.
 */
export function normalizeSyntheticInstagramMetadata(
  identity: CanonicalInstagramContentIdentity,
  syntheticOutput: string
): InstagramSafeMetadata {
  if (identity.platform !== "instagram" || (identity.sourceKind !== "reel" && identity.sourceKind !== "video-post")) {
    throw new TypeError("A canonical Instagram content identity is required.");
  }
  ...
  if (parsed.schemaVersion !== 1 || parsed.platform !== "instagram" || parsed.shortcode !== identity.shortcode) {
    throw metadataError();
  }
  ...
  if (parsed.singleContent !== true) throw metadataError(API_ERROR_CODES.CAROUSEL_NOT_SUPPORTED);
  ...
  return Object.freeze({
    ...
    title: sanitizeText(parsed.title, TITLE_MAX_CODE_POINTS, "Instagram video"),
    caption: sanitizeText(parsed.caption, CAPTION_MAX_CODE_POINTS),
    durationSeconds,
    width,
    height,
